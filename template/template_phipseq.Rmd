...
The absolute number of antibody-bound peptides per individual between groups.
...
The diversity of Ig repertoires of antibody-bound peptides per individual between patient and control groups.
...
Highlighting some virus/bacteria groups

- Adjusted pvals (BH) were calculated using MW test comparing the ratio of group1/group2 of the bacteria/virus of interest and the ratio of group1/group1 of the complete library

```{r}
flags_to_patterns <- list(
  # Flagellins  = c("Flagell", "aureus"),
  Bacteriodales = c("Bacteroida", "Bacteroides", "Prevotella"),
  `Epstein-Barr` = c("Epstein", "EBV"),
...
highlight_colors <- c(
  #"Flagellins" = "#7570b3", 
  "Bacteriodales" = "gold",
  "Epstein-Barr" = "#1b9e77", "Enterovirus" = "#DC000099"
  )
...
Unsupervised learning model try to cluster groups based on jaccard distances between individuals 

- A PERMANOVA pval, if significant, tells us the centroids (big data points) of the groups are further apart than expected by chance.

- A dispersion pval, if significatn, says at least one group is significantly more or less “tightly clustered” around its centroid than the others
...
- Represent the main antigen subgroups ratio distribution within the library (including bacteria/virus of interest) that exhibits differential binding in both groups

- If ratio is positive between group1/group2, this indicate that group1 (eg. Controls) shows more representation of that peptide subgroup than group2 (e.g disease)
...
for (group_col in group_cols) {
  ...
  for (comp_name in comparisons) {
    ...
    group2 <- groups[2]
    comparison_df <- comparison_group_tests[[group_col]][[comp_name]]$comparison_df[
      c("Peptide", "Organism_complete_name", "Description", group1, group2,
    ...
    p <- plot_ratios_by_subgroup(comparison_df, group1, group2, subgroup_lib_df, 
                                 custom_colors = custom_colors, 
                                 subgroup_colors = highlight_colors,
                                 add_subgroups = names(flags_to_patterns),
